...
<section class="content">
    <div class="container-fluid">
        ...
        $cari_rapot= $this->db->query("SELECT * FROM absensi JOIN jadwal ON absensi.id_jadwal = jadwal.id_jadwal WHERE absensi.id_santri = $id_santri[0] AND absensi.status = 1  AND jadwal.id_kelas = $kelas[0]");
       ?>
...
<!-- cari kelas -->
        ...
        ?>
            <div class="card">
                ...
                <div class="row ">
                    ...
                    <div class="col-lg-6">
                        <tr>
                            ...
                            <td>
                                <?php
                                $tampil_kelas = $this->db->query("SELECT * FROM kelas  WHERE id_kelas = $kelas[0]")->row();
                ...
                <div class="body table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <?php
                            $tampil_pelajaran = $this->db->query("SELECT * FROM absensi LEFT JOIN jadwal ON absensi.id_jadwal = jadwal.id_jadwal JOIN pelajaran ON jadwal.id_pelajaran = pelajaran.id_pelajaran WHERE absensi.id_santri = $id_santri[0] AND absensi.status = 1  AND jadwal.id_kelas = $kelas[0] AND pelajaran.jenis = 'Materi' GROUP BY pelajaran.id_pelajaran");
                            $no = 0;
                            $rata_rata = 0;
                            ?>
                            <?php
                            ...
                            ?>
                                ...
                                <?php foreach ($tampil_pelajaran->result() as $tp) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $cari_nilai = $this->db->query("SELECT COALESCE(SUM(nilai), 0) as jumlah, COUNT(nilai) as banyak FROM nilai JOIN jadwal ON nilai.id_jadwal = jadwal.id_jadwal WHERE nilai.id_santri = $id_santri[0] AND nilai.status = 1  AND jadwal.id_pelajaran = $tp->id_pelajaran");
                ...
                <div class="body table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <?php
                            $tampil_pelajaran_p = $this->db->query("SELECT * FROM absensi LEFT JOIN jadwal ON absensi.id_jadwal = jadwal.id_jadwal JOIN pelajaran ON jadwal.id_pelajaran = pelajaran.id_pelajaran WHERE absensi.id_santri = $id_santri[0] AND absensi.status = 1  AND jadwal.id_kelas = $kelas[0] AND pelajaran.jenis = 'Konsep dan Praktikum' GROUP BY pelajaran.id_pelajaran");
                            $no = 0;
                            $rata_rata = 0;
                            ?>
                            <?php
                            ...
                            ?>
                                ...
                                <?php foreach ($tampil_pelajaran_p->result() as $tp) : ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $cari_nilai = $this->db->query("SELECT COALESCE(SUM(nilai), 0) as jumlah, COUNT(nilai) as banyak FROM nilai JOIN jadwal ON nilai.id_jadwal = jadwal.id_jadwal WHERE nilai.id_santri = $id_santri[0] AND jadwal.id_pelajaran = $tp->id_pelajaran AND nilai.status = 1");
                                            if ($cari_nilai->num_rows() > 0) {
                                                $t_nilai = $cari_nilai->row();
                                                $jumlah = $t_nilai->jumlah;
                                                $banyak = $t_nilai->banyak;
                        ...
                        <div class="row">
                                    ...
                                    <div class="col-lg-6">
                                        ...
                                        <tr>
                                            <th>
                                                <?php
                                                    $absen_masuk = $this->db->query("SELECT COUNT(absensi) as masuk FROM absensi WHERE id_santri = $id_santri[0] AND id_kelas = $kelas[0] AND absensi = 'MASUK' AND status = 1")->row();
                                                    echo $absen_masuk->masuk;
                                                ?>
                                            </th>
                                            <th>
                                                <?php
                                                    $absen_ijin = $this->db->query("SELECT COUNT(absensi) as ijin FROM absensi WHERE id_santri = $id_santri[0] AND id_kelas = $kelas[0] AND absensi = 'IJIN' AND status = 1")->row();
                                                    echo $absen_ijin->ijin;
                                                ?>
                                            </th>
                                            <th>
                                                <?php
                                                    $absen_sakit = $this->db->query("SELECT COUNT(absensi) as sakit FROM absensi WHERE id_santri = $id_santri[0] AND id_kelas = $kelas[0] AND absensi = 'SAKIT' AND status = 1")->row();
                                            ...
                                            <th>
                                                <?php
                                                    $absen_alasan = $this->db->query("SELECT COUNT(absensi) as alasan FROM absensi WHERE id_santri = $id_santri[0] AND id_kelas = $kelas[0] AND absensi = 'ALASAN' AND status = 1")->row();
                ...
                <div class="body table-responsive">
                    Berdasarkan hasil penilaian di atas, kepada santri yang bersangkutan dinyatakan  naik kelas
                
                     <?php $naik_kelas = $kelas[0] + 1;
